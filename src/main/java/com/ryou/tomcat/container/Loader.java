...
public class Loader extends URLClassLoader {
    final static Logger log = (Logger) LoggerFactory.getLogger(Loader.class);
    ...
    public Loader(ClassLoader parent, Context cxt) throws Exception {
        ...
        context = cxt;
        this.javaseClassLoader = getSystemClassLoader(); // ApplicationClassLoader，只会加载classpath下的class文件。

        // 进行类加载器的初始化： 一开始就要把 '/WEB-INF/classes/' 、'/WEB-INF/lib/' 这两个地方下的类装入类加载器
        init();
    }
            ...
            if (filenames != null) {
                for (String s : filenames) {
                    ...
                    if (!filename.endsWith(".jar"))
                        continue;
                    File file = new File(webInfLib, s);
